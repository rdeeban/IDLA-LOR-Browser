...
if(retrieve() != null) {
    ...
    var url = '/s3/'+user['bucket']+'/'+user['key'];
    console.log("GET " + url);
    $.ajax({
        url: url,
        type: "GET",
    ...
    nodeEnter.append("svg:text")
        ...
        .on("click", function(d){
            if (d.key.length > 0) {
                ...
                if (user['target'].indexOf(".") != -1) {
                    console.log("is a file");
                    target = (user['target'].split("&")[user['target'].split("&").length-1]);
                }
                else {
                    target = (user['target'].split("&")[user['target'].split("&").length-2]);
                }
                console.log(target);
                $("#target").html(target);
